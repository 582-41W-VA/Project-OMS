...
def createUser(request):
    ...
    If the form is submitted via POST and is valid, saves the new user and redirects to the user list page.
    If the form is not valid or the request method is GET, renders the 'user_form.html' template with the user creation form.
...
def updateUser(request, pk):
    ...
    Renders the user update form initially with the current user data and processes the form submission.
    If the form is submitted via POST and is valid, updates the user's information and redirects to the user details page.
    If the form is not valid or the request method is GET, renders the 'update_user_form.html' template with the user update form pre-filled with the current user data.
    ...
    Returns:
    - Rendered HTML template displaying the user update form or redirecting to the user details page.
    """
    user = get_object_or_404(User, pk=pk)
...
def deleteUser(request, pk):
    ...
    Retrieves the user object with the specified primary key (pk) from the database.
    Checks if the user is a superuser, and if so, displays an error message and redirects to the users list page.
    If the request method is POST, deletes the user from the database and redirects to the users list page.
    If the request method is GET or the user is not a superuser, renders the 'delete_user.html' template with the user details.
    ...
    Returns:
    - Rendered HTML template displaying the delete user confirmation page or redirecting to the users list page.
    """
    user = get_object_or_404(User, pk=pk)
...
def reports(request):
    ...
    Calculates various statistics such as the total number of orders created, pending orders, urgent orders, normal orders,
